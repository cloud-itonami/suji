...
| `deep-squat` | 0.0° | **`:parameter-not-in-source`** — Cho radiographs standing and five *sitting* postures; a squat is not among them. Direction stated: a squat is the posture lordosis is most often reported to reverse in, so a zero here likely **over**-states it |
  ⚠ *Direction now measured, not just stated (found 2026-09-13, bot/suji-anatomy).* Fukushima K, Tsutsumi M, Nakata A, Kakimoto A, Yamaguchi I, Hoshino T, Matsuzaki S, Kudo S, *Relationship between pelvic incidence and hip, pelvic, and lumbar motion during squatting in healthy individuals with high and low pelvic incidence*, **Sci Rep 16:5831, 2026** (doi:10.1038/s41598-026-36815-5, PMID 41559209, PMC12894836; full text read 2026-09-13 through the Europe PMC REST API). 26 healthy men (13 high-PI ≥ 53.8°, 13 low-PI ≤ 43.1°), Vicon 3-D motion analysis, deep bodyweight squat (thigh past parallel, 1-s descent), lumbar curvature from L1–L5 spinous-process markers. The lumbar curvature angle **switches from lordosis to kyphosis during the descent** — the lordosis-to-kyphosis transition sits at 47–53% of the squat cycle in the SPM analysis, with maximum kyphosis at 72–87% — and lumbar excursion over the squat is **38.0 ± 9.3° (low PI) / 33.5 ± 9.9° (high PI)**, positive = lordosis at the start, crossing to negative. That is this README's stated direction, confirmed: a deep squat reverses the lordosis, so the deep-squat entry's 0.0° **over**-states the lumbar lordosis a real squat-bottom holds (by up to the 38° of measured excursion if the reversal completes). It does NOT close the gap: the number is an *excursion from standing*, not an absolute squat-bottom lordosis, it is a 1-s dynamic descent (inertial, not the held static posture this model solves), measured on 22-y-old men with the arms held out front, and the two PI subgroups differ — so `:parameter-not-in-source` stays and **no constant moves**. What changes is that the entry's error direction is no longer an inference from what squats are reported to do; it is measured.
...
| endurance dose (`strain/session-cross-check`), quiet standing | **0 compared / 56** | **9 compared / 56** | **a change, and not obviously an improvement.** Nine muscles now work hard enough to have a finite endurance where none did, and the model/reference ratios are erector spinae 0.573 (`:trunk`), soleus 0.573 and gastrocnemius 0.496 (`:ankle`), gluteus maximus 0.577 (`:trunk`, `:nearest-region`), hamstrings 1.110 (`:knee`). The %MVCs behind them are 14–27%, which is the same overshoot arriving in the strain layer: they are driven by the invented lumbosacral moment and by the ankle and knee moments the translation creates, not by anything about standing. **More rows compared is not more agreement** — nothing here says the nine are right. ⚠ *Annotated 2026-09-23 (bot/suji-anatomy): the "9 compared" count and two of its five named ratios do not reproduce on the tree the row sits on.* Re-measured 2026-09-23 on a fresh clone of `f3ee37c` (the commit this row was last edited in, #102) by driving `strain/session-cross-check` directly through the nbb mirror harness (the kbb suite does not resolve its git deps from a fresh clone — '0 test namespace(s) found', known limitation; the mirror harness is the same one that ran the 2026-09-23 session-cross-check instance counts): the quiet-standing cross-check compares **6 of 56, not 9** — hamstrings 1.110 (`:knee`), gastrocnemius 0.496 (`:ankle`) and soleus 0.573 (`:ankle`), each bilateral, i.e. three leg pairs. **Erector spinae 0.573 and gluteus maximus 0.577 do not appear in the compared set at all**, though the three leg ratios match the prose to three decimals — so the defect is in the count and the two trunk rows, not in the harness. `:could-not-obtain` for the tree that measured 9: the count may predate the 2026-09-11 pelvis/chord changes this same section records, but no provenance for it is named anywhere in the section, and back-searching commits was not done here (measured at `f3ee37c` only). The prose row quoting numbers its own test no longer produces is the same silently-drifting-prose class the `:c2c3` (2026-09-20) and 22.513 N·m → 0.9013 N·m (2026-09-22) annotations record. Suggested owner-side fix: re-derive the row from `session-cross-check` at `HEAD` (6 compared, three bilateral leg pairs) or name the commit where 9 last measured. **No constant moves** — no test edited, no value recomputed, Hansraj 5-value and Wilke sitting 348.86176709999995 N byte-identical (README-only diff) |
